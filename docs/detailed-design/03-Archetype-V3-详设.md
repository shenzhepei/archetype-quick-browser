...
```text
Title bar:  [关闭/最小化/最大化] [当前 Space ▾] [页面 A ×][页面 B ×]... [+]
Toolbar:    [←][→][刷新] [地址栏                         ] [前往]
Bookmarks:  [书签 1] [文件夹 ▾] ...（可隐藏的紧凑栏）
Content:    当前页面
```

- 标签栏直接嵌入原生标题栏，在 macOS 红黄绿按钮或其他平台窗口按钮旁展开；地址栏位于下一行，不在左侧重复展示页面列表。
- 标签项默认最大宽度 220px，随可用空间等宽收缩，最小宽度 72px；关闭按钮固定不参与文本收缩。
- 超过最小宽度容量后标签栏横向滚动或提供溢出菜单，当前标签必须自动保持可见。
- Space 切换入口固定在标题栏系统按钮与标签页之间，显示当前 Space 名称并使用下拉菜单切换；不使用常驻 Space 侧栏。
